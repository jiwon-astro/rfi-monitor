...
OWON HSA1036-TG 스펙트럼 분석기와 Raspberry Pi로 RFI(Radio Frequency Interference)를 장시간 기록하고, Jupyter Notebook에서 spectrum과 time waterfall을 분석하는 도구입니다. 현장에서는 Pi가 측정과 저장을 담당하고, 노트북은 SSH 설정·데이터 전송·분석에 사용합니다.

현재 지원하는 구성은 **HSA1036-TG / firmware V3.0.2.0**, 64-bit Raspberry Pi OS, Python 3입니다. 모델·firmware·serial number와 실제 설정 readback을 확인한 뒤 기록합니다.

## 주요 기능

- 주파수 범위, 최대 bin 간격, RBW/VBW, reference level, attenuation, 내부 preamp 설정.
- Native 801-point sweep을 여러 구간으로 나누어 전체 대역 기록. 선택한 보호 대역은 하나의 sweep에 유지.
- UTC timestamp가 포함된 단일 `raw_sweeps.csv` 저장과 설정·scan plan metadata 기록.
- 측정 시간 지정, SSH 종료 후 background 실행, 상태 확인과 정상 종료.
- 50 Ω 종단기를 이용한 별도 baseline 평균 스펙트럼 측정.
- 완료된 run 중 로컬에 없는 파일만 다운로드하고 SHA-256으로 검증.
- Notebook에서 시간 창별 Max Hold·선형 전력 평균, UTC/KST waterfall, 전체·관심 대역 spectrum 출력.
...
일반 관측은 `record`, 종단기 기준 스펙트럼 측정은 `baseline` 명령으로 실행합니다.
...
SA에는 Positive Peak detector, WRITE trace, single sweep, 자동 sweep time, dBm 단위를 적용합니다. RBW/VBW와 attenuation은 수동값을 사용하고 EMI filter와 TG는 OFF로 설정합니다. 측정 중 SA 전면 조작은 다음 구간 설정에서 덮어써지거나 검증 오류를 일으킬 수 있습니다.
...
겹치는 주파수 좌표는 필요한 구간만 남겨 연결합니다. 값의 보간은 하지 않습니다. `protection_region_hz`는 지정 대역이 한 sweep 안에 들어가게 하며, 예제의 50–400 MHz 보호 대역에서는 bin 간격을 437.5 kHz보다 작게 요구할 수 없습니다. 더 촘촘한 격자는 보호 대역을 줄이거나 `null`로 해제해야 합니다.
...
Pi에서 기록에 필요한 외부 Python 패키지는 `numpy`입니다. 로컬 분석 환경은 저장소 루트에서 설치합니다.

```bash
python -m pip install -r requirements.txt
```

아래 Pi 명령은 `source/`의 실행 모듈들과 사용할 `config.json`이 `~/owon-rfi/`에 배치되어 있고, SA Ethernet 및 SSH 연결이 준비된 구성을 기준으로 합니다. 기존 운영 설치에서는 이 디렉터리를 사용합니다.

먼저 Windows PowerShell에서 저장소 루트를 열고 기록용 UTC clock anchor를 설정합니다. `sync-clock.ps1`의 `Target`, `Key`와 Pi 경로는 자신의 배포 환경에 맞춰 사용합니다. Pi 재부팅 후에는 다시 실행하며, anchor의 유효기간은 7일입니다.

```powershell
.\tools\sync-clock.ps1
```

Pi에서 설정·scan plan을 미리 확인하고, 10분 또는 24시간 기록합니다.
...
SSH 종료 후에도 기록하려면 Pi 전원이 유지되어야 합니다. USB가 유일한 전원이라면 케이블을 분리할 때 Pi도 꺼집니다. 기록 시간은 `--seconds` 또는 `--hours`로 지정하며, 둘 다 생략하면 24시간입니다. 종료 시 진행 중인 cycle 때문에 지정 시간보다 조금 길어질 수 있습니다.
...
`raw_sweeps.csv`의 앞 세 열은 `request_utc_ns`, `read_complete_utc_ns`, `pi_system_request_ns`입니다. 뒤쪽 열 이름에는 실제 주파수가 `<frequency>_Hz` 형식으로 저장됩니다. 첫 두 timestamp는 clock anchor로 계산한 UTC이고, 세 번째는 Pi OS 시각을 확인하기 위한 값입니다.

Windows PowerShell에서 완료된 run을 내려받습니다. `fetch-data.ps1`의 대상 Pi, SSH key, 로컬 저장 경로를 자신의 환경에 맞춥니다.

```powershell
.\tools\fetch-data.ps1 -ListOnly
.\tools\fetch-data.ps1

# 특정 run만 다운로드: 실제 Pi 폴더명으로 바꾸기
...
Notebook에서 `PROJECT_DIR`, `DATA_ROOT`, `INPUTS`를 지정합니다. `INPUTS`에는 관측 run 폴더 또는 `raw_sweeps.csv`를 선택하고, 같은 폴더에 `session.json`과 `metadata_sweeps.json`을 함께 둡니다. Baseline CSV는 이 raw-sweep 입력과 형식이 다릅니다.

분석 설정은 `WINDOW_SECONDS`, `STATISTIC` (`max_dbm` 또는 `mean_peak_power_dbm`), `TIME_ZONE` (`UTC` 또는 `KST`), `FOCUS_MHZ`로 조절합니다. 생성된 그림은 `figures/`에 저장합니다.

## 출력 예시

200 MHz, 약 −40 dBm의 톤 입력을 기록한 예시입니다. 10–1000 MHz를 1601점으로 구성했으며 RBW는 100 kHz입니다. Spectrum은 관측 중 취득한 sweep의 Max Hold와 선형 전력 평균을 함께 보여줍니다.

![200 MHz 톤의 Max Hold 및 평균 spectrum](assets/readme/spectrum.png)

같은 기록의 50–600 MHz 구간을 10초 창별 Max Hold로 표시한 waterfall입니다. 세로축은 KST이며 시간은 아래로 증가합니다.

![10초 Max Hold waterfall, KST](assets/readme/waterfall.png)

출력값은 SA 입력단의 Positive Peak 측정값입니다. 케이블 손실·외부 증폭기 gain·안테나 응답 보정은 적용하지 않습니다. 여러 구간을 순서대로 측정하므로 한 행의 모든 주파수가 동시 측정된 것은 아니며, 평균과 Max Hold는 취득된 sweep에 대한 통계입니다.
